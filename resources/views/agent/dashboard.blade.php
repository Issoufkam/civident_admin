        ...
        <div class="col-lg-6 mb-4">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body d-flex justify-content-center align-items-center">
                    ...
                    {{-- La légende personnalisée a été supprimée car Chart.js la gère maintenant --}}
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    style.innerHTML = `
        .chart-aspect-ratio-container {
            position: relative;
            width: 90%; /* MODIFICATION ICI: Revert à 100% pour remplir l'espace horizontal */
            /* Ajustement de la hauteur relative à la largeur pour contrôler la "longueur" */
            padding-bottom: 40%; /* Maintient le ratio de hauteur souhaité */
            overflow: hidden;
        }
    ...
    const typeChart = new Chart(typeCtx, {
        ...
        options: {
            maintainAspectRatio: false, // Important pour le contrôle de la taille par le parent
            responsive: true, // Par défaut à true dans Chart.js 3+, mais bon de s'assurer
            plugins: {
                legend: {
                    display: true, // Afficher la légende
                    position: 'bottom', // Placer la légende en bas
                    labels: {
                        usePointStyle: true, // Utiliser des points stylisés (cercles) pour la légende
            ...
            cutout: '90%' // Défini à 0% pour un disque complet
        }
    });
    ...
    // Filtrage des demandes (rendu dynamique via AJAX)
    const requestsTableBody = document.querySelector('#requests-table tbody');
    const requestsTableCard = document.querySelector('.col-lg-12 .card'); // Le conteneur parent de la table
    ...
    requestsFilterItems.forEach(item => {
        item.addEventListener('click', async function(e) {
            ...
            const url = this.href; // Utilise l'URL déjà construite par Laravel

            // Crée et affiche l'overlay de chargement
            ...
            `;
            // Assurez-vous que requestsTableCard est positionné relative ou absolute pour que l'overlay fonctionne
            requestsTableCard.style.position = 'relative';
            requestsTableCard.appendChild(loadingOverlay);


            try {
                const response = await fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } }); // Ajoute un header pour indiquer une requête AJAX
                ...
                // Extrait le nouveau tbody et le remplace
                ...
                // Met à jour le texte du bouton du filtre
                ...
                // Supprime toutes les classes 'active' et 'fw-bold' des éléments de la liste
                requestsFilterItems.forEach(item => {
                    item.classList.remove('active', 'fw-bold');
                });
                // Ajoute les classes 'active' et 'fw-bold' à l'élément cliqué
